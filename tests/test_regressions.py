...
class TestMissionCompleteCallbackInterRoomGuard:
    ...
    def test_no_refresh_on_inter_room_charge(self):
        ...
        with patch(
            ...
            cb(self._msg("run", cycle="clean"))
            cb(self._msg("charge", cycle="clean"))
    ...
    def test_no_refresh_on_inter_room_hmpostmsn(self):
        ...
        with patch(
            ...
            cb = make_mission_complete_callback(hass, cc)
            cb(self._msg("run", cycle="clean"))
            cb(self._msg("hmPostMsn", cycle="clean"))
        assert mock_run.call_count == 0
    ...
    def test_no_refresh_on_cycle_quick(self):
        ...
        with patch(
            ...
            cb = make_mission_complete_callback(hass, cc)
            cb(self._msg("run", cycle="quick"))
            cb(self._msg("charge", cycle="quick"))
        assert mock_run.call_count == 0
    ...
    def test_refresh_still_fires_on_genuine_mission_end(self):
        ...
        with patch(
            ...
            cb = make_mission_complete_callback(hass, cc)
            cb(self._msg("run", cycle="clean"))
            cb(self._msg("charge", cycle="none"))
        assert mock_run.call_count == 1, (
            "Regression: genuine mission end must still trigger cloud refresh"
        )
    ...
    def test_refresh_fires_on_stop_genuine_end(self):
        ...
        with patch(
            ...
            cb = make_mission_complete_callback(hass, cc)
            cb(self._msg("run", cycle="clean"))
            cb(self._msg("stop", cycle="none"))
        assert mock_run.call_count == 1

    def test_only_one_refresh_per_multi_room_mission(self):
        """A full 3-room mission with 2 inter-room transitions must fire the
        ...
        with patch(
            ...
            cb = make_mission_complete_callback(hass, cc)
            cb(self._msg("run", cycle="clean"))       # room 1
            cb(self._msg("charge", cycle="clean"))     # inter-room 1→2
            cb(self._msg("run", cycle="clean"))         # room 2
            cb(self._msg("hmPostMsn", cycle="clean"))  # inter-room 2→3
            cb(self._msg("run", cycle="clean"))         # room 3
            cb(self._msg("charge", cycle="none"))       # genuine end
        assert mock_run.call_count == 1
