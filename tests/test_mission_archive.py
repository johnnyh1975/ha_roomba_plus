...
class TestMissionArchiveCorruptionResilience:
    """Stress-test (real-store bug-hunt): async_load survives null records in
    the derived list. Found via field-data stress: derived=[null] made
    _safe_int(r.get(...)) raise AttributeError, uncaught by the original
    ...
    def _load_with(self, payload):
        ...
        with patch("custom_components.roomba_plus.mission_archive.Store",
                   return_value=store_mock):
            asyncio.get_event_loop().run_until_complete(ma.async_load(hass, "e1"))
        return ma

    def test_null_record_in_derived(self):
        ma = self._load_with({
            "derived": [None, {"nMssn": 5}],
            "timeline": [],
            "raw": {},
            "last_nMssn": 0,
        })
        # null record filtered out, no crash
        assert all(isinstance(r, dict) for r in ma._derived)
