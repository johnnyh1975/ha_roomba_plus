...
# ── formerly tests/test_sensor_resilience.py ───────────────────────────────────────
#
...
class TestSmartMapSensorsAreCreated:
    """On a Classic robot with a smart map and a cloud account, setup adds
    the sensors that need both. Nothing covered this path: every sensor
    setup test used an ephemeral map or a Prime entry."""

    def _setup(self, *, smart=True, cloud=True, correlation=True, bbrst=True):
        import asyncio

        from custom_components.roomba_plus import sensor as sensor_mod
        from custom_components.roomba_plus.const import CONF_CORRELATION_ENTITIES
        ...
        d.prime_robot = None
        d.map_capability = MapCapability.SMART if smart else MapCapability.EPHEMERAL
        d.cloud_coordinator = MagicMock() if cloud else None
        d.mission_timer_store = MagicMock()
        d.presence_manager = d.umf_aligner = d.grid_store = None
        reported = {"sku": "R980020"}
        if bbrst:
            reported["bbrstinfo"] = {"nNavRst": 1}
        d.roomba.master_state = {"state": {"reported": reported}}
        entry.options = {CONF_CORRELATION_ENTITIES: ["sensor.dust"]} if correlation else {}
        added: list = []
        asyncio.run(sensor_mod.async_setup_entry(
            MagicMock(), entry, lambda ents, *a, **k: added.extend(ents)))
        return {type(e).__name__ for e in added}

    SMART_AND_CLOUD = {"RoombaLearningPercentageSensor", "RoombaZoneSummarySensor",
                       "RoombaRoomsOverdueSensor", "RoombaMissionProgress"}

    def test_smart_map_and_cloud_add_their_sensors(self):
        namen = self._setup()
        assert self.SMART_AND_CLOUD <= namen
        assert "RoombaDirtCorrelationSensor" in namen
        assert "RoombaResetDiagnosticsSensor" in namen

    def test_without_the_cloud_none_of_them(self):
        namen = self._setup(cloud=False)
        assert not (self.SMART_AND_CLOUD & namen)
        assert "RoombaDirtCorrelationSensor" not in namen

    def test_reset_diagnostics_only_when_the_robot_reports_them(self):
        assert "RoombaResetDiagnosticsSensor" not in self._setup(bbrst=False)
