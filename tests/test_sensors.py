...
# ═══════════════════════════════════════════════════════════════════════
# Merged from test_cloud_history_sensors.py (TEST-REORG, v2.9.1) — cloud
# history sensors (recent_area_30d, recent_time_30d, lifetime_missions):
# mission_history list-response parsing, sqft→m2 / hr+min conversions,
# CLOUD_HISTORY_SENSORS setup-entry gating, MQTT non-update, availability.
# NOTE: _make_sensor/_make_coordinator renamed to _make_history_sensor/
# _make_history_coordinator here to avoid colliding with the differently-
# shaped helpers of the same name already defined earlier in this file.
# ═══════════════════════════════════════════════════════════════════════
...
def _make_history_coordinator(history=None, success=True):
    cc = MagicMock()
...
def _make_history_sensor(key: str, history=None, success=True) -> CloudHistorySensor:
    desc = next(d for d in CLOUD_HISTORY_SENSORS if d.key == key)
    roomba = MagicMock()
    roomba.master_state = {"state": {"reported": {}}}
    cc = _make_history_coordinator(history, success)
