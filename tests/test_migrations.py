...
# ── formerly tests/test_migrations.py ──────────────────────────────────────────────
#
...
class TestNoVersionIsStranded:
    """Every schema version from 1 to 24 must reach 25.

    The two guards next to `test_the_jump_covers_every_early_version`
    assert on the SOURCE of `async_migrate_entry` -- they search it for
    the literal `"3 <= current < 10"`. That passes whether or not the
    chain works: rewriting the condition as `current in range(3, 10)`
    turns them red for no behavioural change, and breaking the condition
    while leaving the string somewhere keeps them green.

    This one runs the chain instead. `async_migrate_entry` advances
    `entry.version` step by step and each branch matches on `==`, so a
    version with no matching branch never advances -- and Home Assistant
    then refuses to load the entry, silently, for anyone who happened to
    be on it.
    """

    def test_every_version_from_one_to_current_arrives_at_25(self):
        import asyncio
        from unittest.mock import MagicMock, patch
        from custom_components.roomba_plus import async_migrate_entry
        ...
        for start in range(1, 25):
            entry = _FakeConfigEntry(version=start, options={})
            hass = _FakeHass()
            fake_reg = MagicMock()
            fake_reg.entities = {}
            loop = asyncio.new_event_loop()
            try:
                ...
                loop.close()
            if entry.version != 25:
                stranded.append(start)

        assert not stranded, (
            f"entries starting at {stranded} did not reach version 25. "
            "An entry that stops short never loads — the user sees the "
            "integration fail to set up with no indication why."
        )


# ── formerly tests/test_migration_registry.py ───────────────────────────────────
#
# Entity-registry migrations, run against the REAL registry.
#
# Each schema step renames entities: localised ids to English suffixes, old
# names to new. The existing migration tests patch the registry; these
# register real entities on a real config entry at the old version, run the
# whole chain to the current version, and check what a user who upgrades
# actually ends up with.

BLID = "MIGRATEBLID1"


def _entry(hass, version, **options):
    e = MockConfigEntry(domain=DOMAIN, version=version, unique_id=BLID,
                        data={"blid": BLID, "host": "10.0.0.9", "password": "pw"},
                        options=options)
    e.add_to_hass(hass)
    return e


def _add(hass, entry, domain, uid_tail, object_id):
    reg = er.async_get(hass)
    return reg.async_get_or_create(domain, DOMAIN, f"{BLID}{uid_tail}",
                                   suggested_object_id=object_id, config_entry=entry).entity_id


def _eid(hass, domain, uid_tail):
    return er.async_get(hass).async_get_entity_id(domain, DOMAIN, f"{BLID}{uid_tail}")


_LOOKALIKES = {
    "sensor": ["hue_lifetime_area", "other_cloud_recent_area_30d", "zigbee_wartung_bursten",
               "plug_mission_recharge_time", "meter_battery", "bath_cloud_recent_dirt_density"],
    "image": ["camera_map"],
}


class TestV10LifetimeToRecent:

    @pytest.mark.asyncio
    async def test_lifetime_ids_become_recent_30d(self, hass):
        entry = _entry(hass, 10)
        _add(hass, entry, "sensor", "_a", "robbie_lifetime_area")
        _add(hass, entry, "sensor", "_t", "robbie_lifetime_time")
        assert await async_migrate_entry(hass, entry) is True
        assert _eid(hass, "sensor", "_a") == "sensor.robbie_recent_area_30d"
        assert _eid(hass, "sensor", "_t") == "sensor.robbie_recent_time_30d"

    @pytest.mark.asyncio
    async def test_an_occupied_target_removes_the_old_entity(self, hass):
        """The new id already exists: the stale one goes, the history of
        the existing one stays."""
        entry = _entry(hass, 10)
        _add(hass, entry, "sensor", "_new", "robbie_recent_area_30d")
        _add(hass, entry, "sensor", "_old", "robbie_lifetime_area")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_old") is None
        assert _eid(hass, "sensor", "_new") == "sensor.robbie_recent_area_30d"


class TestV11CloudSuffixes:

    @pytest.mark.asyncio
    async def test_a_localised_cloud_sensor_gets_its_english_suffix(self, hass):
        entry = _entry(hass, 11)
        # v11 learns the device prefix from a CLOUD sibling whose id is
        # already right: uid "..._cloud_<key>", entity id "..._<key>".
        _add(hass, entry, "sensor", "_cloud_lifetime_missions", "robbie_lifetime_missions")
        _add(hass, entry, "sensor", "_cloud_recent_area_30d", "robbie_flache_30_tage")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_cloud_recent_area_30d") == "sensor.robbie_recent_area_30d"


class TestV13EnglishSuffixes:

    @pytest.mark.asyncio
    async def test_a_localised_recharge_sensor_is_renamed(self, hass):
        entry = _entry(hass, 13)
        _add(hass, entry, "sensor", "_lifetime_missions", "robbie_lifetime_missions")
        _add(hass, entry, "sensor", "_mission_recharge_time", "robbie_ladezeit")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_mission_recharge_time") == "sensor.robbie_recharge_time"

    @pytest.mark.asyncio
    async def test_already_english_is_left_alone(self, hass):
        entry = _entry(hass, 13)
        _add(hass, entry, "sensor", "_signal_noise", "robbie_signal_noise")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_signal_noise") == "sensor.robbie_signal_noise"

    @pytest.mark.asyncio
    async def test_no_way_to_find_the_device_prefix_leaves_it(self, hass):
        """Without a sibling to learn the prefix from, guessing would risk
        a wrong id; the entity keeps its name."""
        entry = _entry(hass, 13)
        _add(hass, entry, "sensor", "_mission_recharge_time", "ladezeit")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_mission_recharge_time") == "sensor.ladezeit"


class TestV20CloudPrefix:

    @pytest.mark.asyncio
    async def test_cloud_prefixed_ids_are_fixed(self, hass):
        entry = _entry(hass, 20)
        _add(hass, entry, "sensor", "_cloud_recent_dirt_density", "robbie_cloud_recent_dirt_density")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_cloud_recent_dirt_density") == "sensor.robbie_recent_dirt_density"


class TestV21NoLongerCrashes:
    """`new_eid = ""` with `is None` checks sent every entity matching no
    cloud suffix to `async_update_entity(eid, new_entity_id="")`; Home
    Assistant raised ValueError and the whole migration aborted. Anyone on
    schema 20 or older could not load the integration after upgrading."""

    @pytest.mark.asyncio
    async def test_an_ordinary_entity_survives_the_step(self, hass):
        entry = _entry(hass, 20)
        _add(hass, entry, "vacuum", "_vac", "robbie")
        assert await async_migrate_entry(hass, entry) is True
        assert _eid(hass, "vacuum", "_vac") == "vacuum.robbie"

    @pytest.mark.asyncio
    async def test_the_battery_and_map_renames_now_run(self, hass):
        entry = _entry(hass, 20)
        _add(hass, entry, "sensor", "_bat", "robbie_battery")
        _add(hass, entry, "image", "_map", "robbie_map")
        _add(hass, entry, "image", "_cov", "robbie_coverage_map")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_bat") == "sensor.robbie_battery_level"
        assert _eid(hass, "image", "_map") == "image.robbie_cleaning_map"
        assert _eid(hass, "image", "_cov") == "image.robbie_coverage_map", "already right: untouched"


@pytest.mark.parametrize("version", list(range(10, 21)))
@pytest.mark.asyncio
async def test_a_foreign_integrations_entities_are_never_renamed(hass, version):
    """Another integration's entity whose name happens to end in a suffix
    a step looks for must come through every step untouched."""
    entry = _entry(hass, version)
    reg = er.async_get(hass)
    fremde = {}
    for domain, names in _LOOKALIKES.items():
        for name in names:
            fremde[name] = reg.async_get_or_create(domain, "other_integration", f"x_{name}",
                                                   suggested_object_id=name).entity_id
    _add(hass, entry, "vacuum", "_vac", "robbie")
    assert await async_migrate_entry(hass, entry) is True
    for name, eid in fremde.items():
        assert reg.async_get(eid) is not None, f"{eid} was renamed or removed at v{version}"


class TestV21Zombie:

    @pytest.mark.asyncio
    async def test_an_occupied_target_is_cleared_then_renamed_into(self, hass):
        """A leftover entity already holds the correct id: it is removed so
        the real sensor can take the name."""
        entry = _entry(hass, 20)
        _add(hass, entry, "sensor", "_zombie", "robbie_recent_dirt_density")
        _add(hass, entry, "sensor", "_real", "robbie_cloud_recent_dirt_density")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_zombie") is None
        assert _eid(hass, "sensor", "_real") == "sensor.robbie_recent_dirt_density"


class TestV16DeviceNamedIds:
    """Ids are rebuilt from the device name and the unique id — the German
    ids of early releases become '<device>_<english key>'."""

    def _device(self, hass, entry, name="Robbie"):
        from homeassistant.helpers import device_registry as dr

        return dr.async_get(hass).async_get_or_create(
            config_entry_id=entry.entry_id, identifiers={(DOMAIN, BLID)}, name=name)

    def _add_on(self, hass, entry, device, uid_tail, object_id, domain="sensor"):
        return er.async_get(hass).async_get_or_create(
            domain, DOMAIN, f"{BLID}{uid_tail}", suggested_object_id=object_id,
            config_entry=entry, device_id=device.id).entity_id

    @pytest.mark.asyncio
    async def test_a_german_id_becomes_device_plus_english_key(self, hass):
        entry = _entry(hass, 16)
        dev = self._device(hass, entry)
        self._add_on(hass, entry, dev, "_brush_remaining_hours", "robbie_wartung_bursten")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_brush_remaining_hours") == "sensor.robbie_brush_remaining_hours"

    @pytest.mark.asyncio
    async def test_an_occupied_expected_id_is_not_taken(self, hass):
        """Another entity already has the name: keep both, do not steal."""
        entry = _entry(hass, 16)
        dev = self._device(hass, entry)
        er.async_get(hass).async_get_or_create("sensor", "other_integration", "x",
                                               suggested_object_id="robbie_total_missions")
        self._add_on(hass, entry, dev, "_total_missions", "robbie_missionen_gesamt")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_total_missions") == "sensor.robbie_missionen_gesamt"

    @pytest.mark.asyncio
    async def test_without_a_device_the_id_is_left(self, hass):
        entry = _entry(hass, 16)
        _add(hass, entry, "sensor", "_total_missions", "missionen_gesamt")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_total_missions") == "sensor.missionen_gesamt"

    @pytest.mark.asyncio
    async def test_a_user_named_device_gives_its_name(self, hass):
        from homeassistant.helpers import device_registry as dr

        entry = _entry(hass, 16)
        dev = self._device(hass, entry)
        dr.async_get(hass).async_update_device(dev.id, name_by_user="Wohnzimmer Sauger")
        self._add_on(hass, entry, dev, "_total_missions", "robbie_missionen_gesamt")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_total_missions") == "sensor.wohnzimmer_sauger_total_missions"


class TestV11FallbackPrefix:

    @pytest.mark.asyncio
    async def test_the_prefix_is_found_across_the_registry(self, hass):
        """No sibling among the wrong ones: the prefix comes from any cloud
        entity of the same robot whose id is already right."""
        entry = _entry(hass, 11)
        _add(hass, entry, "sensor", "_cloud_recent_time_30d", "robbie_zeit_30_tage")
        _add(hass, entry, "sensor", "_cloud_lifetime_missions", "robbie_lifetime_missions")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_cloud_recent_time_30d") == "sensor.robbie_recent_time_30d"


class TestV11Edges:

    @pytest.mark.asyncio
    async def test_no_correct_sibling_anywhere_keeps_the_entity(self, hass, caplog):
        entry = _entry(hass, 11)
        _add(hass, entry, "sensor", "_cloud_recent_area_30d", "flache_30_tage")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_cloud_recent_area_30d") == "sensor.flache_30_tage"

    @pytest.mark.asyncio
    async def test_a_taken_target_removes_the_wrong_copy(self, hass):
        entry = _entry(hass, 11)
        _add(hass, entry, "sensor", "_cloud_lifetime_missions", "robbie_lifetime_missions")
        er.async_get(hass).async_get_or_create("sensor", DOMAIN, "leftover",
                                               suggested_object_id="robbie_recent_area_30d",
                                               config_entry=entry)
        _add(hass, entry, "sensor", "_cloud_recent_area_30d", "robbie_flache_30_tage")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_cloud_recent_area_30d") is None


class TestV16GermanEdges:

    def _dev(self, hass, entry, name):
        from homeassistant.helpers import device_registry as dr

        return dr.async_get(hass).async_get_or_create(
            config_entry_id=entry.entry_id, identifiers={(DOMAIN, BLID)}, name=name)

    def _on(self, hass, entry, uid_tail, object_id, device=None):
        return er.async_get(hass).async_get_or_create(
            "sensor", DOMAIN, f"{BLID}{uid_tail}", suggested_object_id=object_id,
            config_entry=entry, device_id=device.id if device else None).entity_id

    @pytest.mark.asyncio
    async def test_a_german_id_without_a_device_is_removed_by_the_cleanup_step(self, hass):
        entry = _entry(hass, 16)
        self._on(hass, entry, "_x", "robbie_wartung_bursten")
        await async_migrate_entry(hass, entry)
        # Step 16 cannot rename it (no usable device name), so step 18
        # removes it as a stale German entry, by design: the sensor then
        # registers itself afresh under its English id. A separate step,
        # because HA reuses a just-deleted id within the same session.
        assert _eid(hass, "sensor", "_x") is None

    @pytest.mark.asyncio
    async def test_a_device_name_without_letters_is_removed_by_the_cleanup_step(self, hass):
        entry = _entry(hass, 16)
        dev = self._dev(hass, entry, "!!!")
        self._on(hass, entry, "_x", "robbie_wartung_bursten", dev)
        await async_migrate_entry(hass, entry)
        # Step 16 cannot rename it (no usable device name), so step 18
        # removes it as a stale German entry, by design: the sensor then
        # registers itself afresh under its English id. A separate step,
    ...
    @pytest.mark.asyncio
    async def test_a_taken_english_target_removes_the_german_copy(self, hass):
        entry = _entry(hass, 16)
        dev = self._dev(hass, entry, "Robbie")
        self._on(hass, entry, "_brush_remaining_hours", "robbie_brush_remaining_hours", dev)
        self._on(hass, entry, "_old_german", "robbie_wartung_bursten", dev)
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_old_german") is None
        assert _eid(hass, "sensor", "_brush_remaining_hours") == "sensor.robbie_brush_remaining_hours"

    @pytest.mark.asyncio
    async def test_an_old_format_unique_id_is_left(self, hass):
        """A unique id not starting with the BLID cannot give a key safely."""
        entry = _entry(hass, 16)
        dev = self._dev(hass, entry, "Robbie")
        eid = er.async_get(hass).async_get_or_create(
            "sensor", DOMAIN, "legacy-format-id", suggested_object_id="robbie_irgendwas",
            config_entry=entry, device_id=dev.id).entity_id
        await async_migrate_entry(hass, entry)
        assert er.async_get(hass).async_get(eid) is not None


class TestTakenTargetsAcrossSteps:
    """In every renaming step: when the correct id is already held by the
    current entity, the stale copy is removed rather than renamed onto it."""

    @pytest.mark.asyncio
    async def test_v10_time(self, hass):
        entry = _entry(hass, 10)
        _add(hass, entry, "sensor", "_new", "robbie_recent_time_30d")
        _add(hass, entry, "sensor", "_old", "robbie_lifetime_time")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_old") is None

    @pytest.mark.asyncio
    async def test_v12_map(self, hass):
        entry = _entry(hass, 12)
        _add(hass, entry, "sensor", "_lifetime_missions", "robbie_lifetime_missions")
        er.async_get(hass).async_get_or_create("image", DOMAIN, "current-map",
                                               suggested_object_id="robbie_cleaning_map",
                                               config_entry=entry)
        _add(hass, entry, "image", "_map", "robbie_karte")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "image", "_map") is None

    @pytest.mark.asyncio
    async def test_v13_recharge(self, hass):
        entry = _entry(hass, 13)
        _add(hass, entry, "sensor", "_lifetime_missions", "robbie_lifetime_missions")
        er.async_get(hass).async_get_or_create("sensor", DOMAIN, "current-recharge",
                                               suggested_object_id="robbie_recharge_time",
                                               config_entry=entry)
        _add(hass, entry, "sensor", "_mission_recharge_time", "robbie_ladezeit")
        await async_migrate_entry(hass, entry)
        assert _eid(hass, "sensor", "_mission_recharge_time") is None


class TestPrefixSearchIgnoresStrangers:

    @pytest.mark.parametrize("version", [12, 13])
    @pytest.mark.asyncio
    async def test_foreign_and_old_format_entries_do_not_supply_a_prefix(self, hass, version):
        """The device prefix is learned only from this robot's own
        entities; a foreign entity or an old-format id must not lend it."""
        entry = _entry(hass, version)
        reg = er.async_get(hass)
        reg.async_get_or_create("sensor", "other_integration", f"{BLID}_lifetime_missions",
                                suggested_object_id="wrong_lifetime_missions")
        reg.async_get_or_create("sensor", DOMAIN, "oldformat_lifetime_missions",
                                suggested_object_id="old_lifetime_missions", config_entry=entry)
        _add(hass, entry, "sensor", "_lifetime_missions", "robbie_lifetime_missions")
        tail = "_raw_state" if version == 12 else "_mission_recharge_time"
        _add(hass, entry, "sensor", tail, "robbie_rohdaten" if version == 12 else "robbie_ladezeit")
        await async_migrate_entry(hass, entry)
        got = _eid(hass, "sensor", tail)
        assert got is None or got.startswith("sensor.robbie_"), got
