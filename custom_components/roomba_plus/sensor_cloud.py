...
# Role -> translation key. An id whose role this integration does not know
# still gets an entity, via _PART_GENERIC_KEY plus a part_id placeholder —
# a robot family reporting unfamiliar ids degrades to "unnamed but working"
# rather than to nothing at all.
_PART_ROLE_TRANSLATION_KEYS: dict[str, str] = {
    IROBOT_PART_ROLE_FILTER:         "part_filter",
    IROBOT_PART_ROLE_SIDE_BRUSH:     "part_side_brush",
    IROBOT_PART_ROLE_MAIN_BRUSH:     "part_main_brush",
    IROBOT_PART_ROLE_CLEAN_BASE_BAG: "part_clean_base_bag",
}
_PART_GENERIC_KEY = "part_generic"

_MINUTES_PER_HOUR = 60


class CloudPartSensor(IRobotEntity, SensorEntity):
    """Hours of robot runtime left on one consumable, straight from iRobot.

    This is the same figure the official app's maintenance tiles show,
    because it is the same source: `count_remaining` from
    /v1/robots/{blid}/parts, denominated in robot runtime minutes.

    Keyed on `part_id`, NOT on the role. The id is what iRobot guarantees;
    the id->role mapping is this integration's inference (see
    const.IROBOT_PART_ROLES). Pinning unique_id to the id means correcting
    a role later renames an entity instead of orphaning it and creating a
    duplicate.

    One sensor exists per part the cloud reports, so a robot with a Clean
    Base reports four and one without reports three, with no per-model
    entity list to maintain here.
    """
    ...
    def __init__(
        ...
        self._role = role
        self._attr_unique_id = f"{self.robot_unique_id}_cloud_part_{self._part_id}"
        self._attr_translation_key = (
            _PART_ROLE_TRANSLATION_KEYS.get(role or "") or _PART_GENERIC_KEY
        )
        if self._attr_translation_key == _PART_GENERIC_KEY:
            self._attr_translation_placeholders = {"part_id": self._part_id}

    def _record(self) -> dict[str, Any] | None:
        """Return this part's entry from the latest cloud fetch."""
        for entry in self._coordinator.parts:
            if str(entry.get("part_id")) == self._part_id:
                return entry
        return None
    ...
    def native_value(self) -> StateType:
        ...
        if record is None:
            return None
        # Only minute-denominated counters convert to hours. Any other
        # count_type is reported as-is by iRobot and would be silently
        # wrong divided by 60.
        if record.get("count_type") != "minutes":
            return None
        ...
        if remaining is None:
            return None
        try:
            return round(int(remaining) / _MINUTES_PER_HOUR, 1)
        except (TypeError, ValueError):
            return None

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Expose the raw counters, so the id->role inference is checkable.

        `part_id` and `role` are here deliberately: comparing one screen of
        the official app against these attributes is all it takes to
        confirm or correct the mapping in const.IROBOT_PART_ROLES, which is
        field-inferred rather than vendor-documented.
        """
        record = self._record()
        if record is None:
            return {}
        ...
        attrs: dict[str, Any] = {
            ...
            "role_source": "inferred" if self._role else "unmapped",
            "count_type": record.get("count_type"),
            "minutes_used": used,
            "minutes_remaining": remaining,
            "percent_used": record.get("counter"),
            "counter_category": record.get("counter_category"),
            "reset_by": record.get("reset_by"),
            "source": "irobot_cloud_parts",
        }
        if used is None or remaining is None:
            attrs["budget_hours"] = None
        else:
            try:
                attrs["budget_hours"] = round(
                    (int(used) + int(remaining)) / _MINUTES_PER_HOUR, 1
                )
            except (TypeError, ValueError):
                attrs["budget_hours"] = None
        last_ts = record.get("last_updated_ts")
        if last_ts:
            try:
                attrs["last_replaced"] = dt_util.utc_from_timestamp(
                    int(last_ts)
                ).isoformat()
            except (TypeError, ValueError, OSError):
                attrs["last_replaced"] = None
        else:
            # No timestamp means this part has never been reset — the
            # counter has been running since the robot's first use.
            attrs["last_replaced"] = None
            attrs["never_reset"] = True
        return attrs
    ...
    def new_state_filter(self, new_state: dict[str, Any]) -> bool:
        # Cloud-sourced; MQTT messages carry nothing for this sensor.
        return False
...
def build_cloud_part_sensors(
    roomba: Any, blid: str, coordinator: IrobotCloudCoordinator
) -> list[CloudPartSensor]:
    """Return one CloudPartSensor per consumable the cloud reports.

    Empty list when the robot or account does not serve the parts endpoint,
    which is the same shape as a failed fetch — both mean "create nothing"
    rather than "create placeholders".
    """
    sensors: list[CloudPartSensor] = []
    for entry in coordinator.parts:
        part_id = entry.get("part_id")
        if part_id is None:
            continue
        sensors.append(
            CloudPartSensor(
                roomba, blid, coordinator, str(part_id), part_role(part_id)
            )
        )
    return sensors
